      ...
      <table class="tableAction ajaxlist">
        ...
        <tr>
            ...
            @if (isset(App\Actions_plan::setAllDay($list2->id)->if_all_day) and   App\Actions_plan::setAllDay($list2->id)->if_all_day == 1)
                <td class="tdAction"><div class="center colorLevel_percent colorLevel_percent_{{App\Http\Services\Action::setColorPercent($list2->percent_executing  ,2)}}">{{round($list2->percent_executing,2)}} %</td>
            @else
                <td class="tdAction"><div class="center colorLevel_percent colorLevel_percent_{{App\Http\Services\Action::setColorPercent($list2->percent_executing,2)}}">{{round($list2->percent_executing,2)}} %</td>
            @endif
