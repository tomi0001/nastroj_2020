      ...
      @if ($day2 <= $how_day_month ) 
        ...
        @if ($day1 >= $day_week )
            ...
            @else
                ...
                <div onmouseover='calendarOn("day_{{$day2}}")' onmouseout='calendarOff("day_{{$day2}}")' align=center id='day_{{$day2}}' class="cell{{$color[$day2-1]}}" onclick="LoadPage('{{route('Drmain')}}/{{$year}}/{{$month}}/{{$day2}}')"><a  class="no_active" href={{   route('Drmain')}}/{{$year}}/{{$month}}/{{$day2}}  }}>{{$day2}}</a></div>
...
<div class="row center">
  ...
  <div class="col-md-2 col-xs-2">
      <button class="btn btn-primary" onclick=location.href="{{ route('Drmain')}}/{{$back[0]}}/{{$back[1]}}/1/wstecz">Miesiąc Wstecz</button>
  </div>
  <div class="col-md-2 col-xs-2">
          <button class="btn btn-primary" onclick=location.href="{{ route('Drmain')}}/{{$back_year[0]}}/{{$back_year[1]}}/1/wstecz">Rok Wstecz</button>
  </div>
  <div class="col-md-2 col-xs-2">
      <button class="btn btn-primary" onclick=location.href="{{ route('Drmain')}}/{{$next_year[0]}}/{{$next_year[1]}}/1/wstecz">Rok Dalej</button>
      
  </div>
  <div class="col-md-2 col-xs-2">
      <button class="btn btn-primary" onclick=location.href="{{ route('Drmain')}}/{{$next[0]}}/{{$next[1]}}/1/wstecz">miesiąc Dalej</button>
